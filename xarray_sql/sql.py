        ...
        *,
        ...
        Args:
            ...
            index_columns: When True, add an ``int32`` ``<dim>_idx`` column for
                ...
                float-equality mismatches. They also carry partition-pruning
                bounds, so ``WHERE <dim>_idx IN (...)`` prunes partitions like a
                dimension coordinate. The coordinate columns stay available for
                value predicates and display; index columns are off by default.
