...
def build_model_with_table_names(
    init_weight: Callable[[int, int], np.array], widths=(28 * 28, 196, 32, 10)
    ...
    ds = xr.Dataset(weights, coords=coords).expand_dims(
        {"layer": np.arange(len(weights))}
    )
...
def main():
    ...
    ctx.from_dataset(
        "model", model, table_names=table_names, chunks=dict(layer=1)
    )
